...
class HudongBaike:
    def get_html(self, url):
        ...
        return request.urlopen(req).read().decode('utf-8').replace('&nbsp;', '')
    ...
    def checkhudong_polysemantic(self, selector):
        semantics = ['http:' + sem for sem in selector.xpath("//ul[@id='polysemyAll']/li/a/@href") if 'doc_title' not in sem]
...
class SougouBaike:
    ...
    def find_sofouid(self, word):
        ...
        # url = "https://baike.sogou.com/v14085.htm?fromTitle=%s" % parse.quote(word)
        print(url)
        selector = etree.HTML(self.get_html(url))
        id = selector.xpath('//h3/a/@href')[0].split(';')[0]
        info_url = "https://www.sogou.com%s"%id
        print(info_url)
        return info_url
    ...
    def info_extract_sogou(self, word):
        ...
        url_info = pattern.findall(result.decode('utf-8'))[0]
        url_selector = etree.HTML(self.get_html(url_info))
        info_list = list()
        info_data = self.extract_sogou(url_selector)
        if url_selector.xpath('//li[@class="current_item"]/text()'):
            info_data['current_semantic'] = url_selector.xpath('//li[@class="current_item"]/text()')[0].replace('    ', '').replace('（','').replace('）','')
        else:
            info_data['current_semantic'] = ''

        info_list.append(info_data)
        # polysemantics = self.checksogou_polysemantic(url_selector)
        # if polysemantics:
        #     info_list += polysemantics
        infos = [info for info in info_list if len(info) > 2]
        return infos
    ...
    def checksogou_polysemantic(self, selector):
        semantics = ['http://www.sogou.com' + sem.split('?')[0] for sem in selector.xpath("//ol[@class='semantic_item_list']/li/a/@href")]
